...
```json
{
    "ipWhitelistEnable":true, // IP白名单是否启用
    "ipWhitelist":["127.0.0.1"], // IP白名单列表
    "gasManageEnable":true, // 点对点转账管理是否启用，启用后只有燃料管理员账户可以收发燃料
    "gasManageAccounts":["0x2D3913c12ACa0E4A2278f829Fb78A682123c0125"], // 燃料管理员账户地址列表
    "blacklistEnable":true, // 账户（合约）黑名单是否启用，启用后黑名单地址不能发送和接收交易
